import { Building2, ExternalLink, Github, LogIn, LogOut, MapPinned } from 'lucide-react';
import { NavLink, Outlet, useNavigate } from 'react-router-dom';
import { useState } from 'react';
import { useAuth } from '../auth/authContext';
import { API_ORIGIN, INTERNAL_DASHBOARD_URL, PARTNER_PORTAL_ENABLED, SHOW_API_ORIGIN } from '../config/env';
...
export default function AppShell() {
  ...
  const activeMemberships = auth.user?.memberships.filter((membership) => membership.isActive) ?? [];
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-ink">
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
          ...
          <div className="flex items-center gap-2 text-sm">
            {SHOW_API_ORIGIN ? (
              <span className="hidden max-w-80 truncate rounded-md border border-slate-200 bg-slate-100 px-3 py-2 text-slate-600 md:inline">
                API: {API_ORIGIN ? API_ORIGIN.replace(/^https?:\/\//, '') : 'same origin'}
              </span>
            ) : null}
            <a
            ...
            {auth.status === 'authenticated' ? (
              <>
                ...
                ) : auth.user?.isPlatformAdministrator && INTERNAL_DASHBOARD_URL ? (
                  <a
                    href={INTERNAL_DASHBOARD_URL}
                    className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 font-extrabold text-slate-700 transition hover:border-slate-300 hover:bg-slate-100"
                  >
                    <ExternalLink size={17} aria-hidden="true" />
                    <span className="hidden sm:inline">Sunbird internal dashboard</span>
                  </a>
                ) : null}
                <button
                  type="button"
            ...
            ) : auth.status === 'anonymous' && PARTNER_PORTAL_ENABLED ? (
              <NavLink
                to="/login"
        ...
        {auth.status === 'error' ? (
          <div className="border-t border-amber-100 bg-amber-50 px-4 py-2 text-center text-sm text-amber-900" role="status">
            Authentication is temporarily unavailable. Public pages remain available.
          </div>
        ) : null}
      </header>
      <main>
